...
def imagestream(
    ...
    while True:
        ...
        ret, jpg = cv2.imencode(".jpg", frame, [int(cv2.IMWRITE_JPEG_QUALITY), 96])
        yield (
            b"--frame\r\n"
...
def event_thumbnail(
    ...
    if format == "android":
        ...
        if extension in (Extension.jpg, Extension.jpeg):
            quality_params = [int(cv2.IMWRITE_JPEG_QUALITY), 96]
        elif extension == Extension.webp:
...
def label_snapshot(request: Request, camera_name: str, label: str):
    ...
    try:
        ...
        _, jpg = cv2.imencode(".jpg", frame, [int(cv2.IMWRITE_JPEG_QUALITY), 96])
