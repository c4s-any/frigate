...
def events(params: EventsQueryParams = Depends()):
    ...
    if sub_labels != "all":
        ...
        for label in filtered_sub_labels:
            ...
            # include this label when part of a comma-separated list
            sub_label_clauses.append((Event.sub_label.cast("text") % f"*{label},*"))
            sub_label_clauses.append((Event.sub_label.cast("text") % f"*, {label}*"))
            
            # include this label when part of a space-separated list (for vehicle ReID features)
            sub_label_clauses.append((Event.sub_label.cast("text") % f"*{label} *"))
            sub_label_clauses.append((Event.sub_label.cast("text") % f"* {label}*"))
            
            # include this label when part of a hyphen-separated list (for person ReID features)
            sub_label_clauses.append((Event.sub_label.cast("text") % f"*{label}-*"))
            sub_label_clauses.append((Event.sub_label.cast("text") % f"*-{label}*"))
