...
def events(params: EventsQueryParams = Depends()):
    ...
    if description != "all":
        ...
        # with recognized license plates 'ABC123' and 'ABC123, XYZ789'
        description_clauses = []
        filtered_descriptions = description.split(",")

        if "None" in filtered_descriptions:
            filtered_descriptions.remove("None")
            description_clauses.append(
                (Event.data["description"].is_null())
            )
        ...
        for description in filtered_descriptions:
            ...
            description_clauses.append(
                (
                    Event.data["description"].cast("text")
                    == description
                )
            )
            description_clauses.append(
        ...
        description_clause = reduce(
            operator.or_, description_clauses
        )
        clauses.append((description_clause))
...
def events_search(request: Request, params: EventsSearchQueryParams = Depends()):
    ...
    if description != "all":
        ...
        event_filters.append((description_clause))
