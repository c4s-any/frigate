...
def get_descriptions(split_joined: Optional[int] = None):
    try:
        events = Event.select(Event.data).distinct()
    except Exception:
        return JSONResponse(
    ...
    descriptions = []
    for e in events:
        if e.data is not None and "description" in e.data:
            descriptions.append(e.data["description"])

    while None in descriptions:
        descriptions.remove(None)

    if split_joined:
        original_descriptions = descriptions.copy()
        for description in original_descriptions:
            if description and "," in description:
                descriptions.remove(description)
                parts = description.split(",")
                for part in parts:
                    if part.strip() not in descriptions:
