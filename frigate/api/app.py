...
def get_sub_labels(split_joined: Optional[int] = None):
    ...
    if split_joined:
        ...
        for label in original_labels:
            # Handle comma-separated labels (original logic)
            if "," in label:
            ...
            # Handle space-separated labels (for vehicle ReID features)
            ...
            # Handle hyphen-separated labels (for person ReID features)
            elif "-" in label and len(label.split("-")) > 1:
                sub_labels.remove(label)
                parts = label.split("-")
