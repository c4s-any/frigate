...
def stats_snapshot(
    ...
    if embeddings_metrics:
        ...
        if config.preid.enabled:
            stats["embeddings"]["person_reid_speed"] = round(
                embeddings_metrics.preid_speed.value * 1000, 2
            )
            stats["embeddings"]["person_reid"] = round(
                embeddings_metrics.preid_pps.value, 2
            )

        if config.vreid.enabled:
            stats["embeddings"]["vehicle_reid_speed"] = round(
                embeddings_metrics.vreid_speed.value * 1000, 2
            )
            stats["embeddings"]["vehicle_reid"] = round(
                embeddings_metrics.vreid_pps.value, 2
            )
