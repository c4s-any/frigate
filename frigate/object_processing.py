...
# REID
from frigate.initialize_reid_session import initialize_onnx_session
...
from frigate.const import CLIPS_DIR, UPDATE_CAMERA_ACTIVITY, MODEL_P_REID_PATH, MODEL_V_REID_PATH
...
class TrackedObjectProcessor(threading.Thread):
    def __init__(
        ...
        ort_session_p = initialize_onnx_session(MODEL_P_REID_PATH)
        ort_session_v = initialize_onnx_session(MODEL_V_REID_PATH)
        ...
        def end(camera: str, obj: TrackedObject, frame_name: str):
            ...
            # 获取配置
            lpid_config: LpidConfig = self.config.lpid
            reid_config: ReidConfig = self.config.reid
            ...
            # 属性识别
            if reid_config.p_enabled or reid_config.v_enabled:
                attr_jpg_bytes = obj.get_person_vehicle_attr(
                    ort_session_p, ort_session_v, lpid_config.enabled, reid_config.p_enabled,
                    reid_config.v_enabled, reid_config.shorter_size, reid_config.longer_size,
            ...
            if obj.has_snapshot:
                ...
                if jpg_bytes is None:
                    ...
                    if attr_jpg_bytes is not None:
                        ...
                        snapshot_bytes = jpg_bytes
                    snapshot_path = os.path.join(CLIPS_DIR, f"{camera}-{obj.obj_data['id']}.jpg")
                    try:
                        with open(snapshot_path, "wb") as j:
                            j.write(snapshot_bytes)
                ...
                if snapshot_config.clean_copy:
                    ...
                    if png_bytes is None:
                        ...
                        clean_snapshot_path = os.path.join(
                            CLIPS_DIR, f"{camera}-{obj.obj_data['id']}-clean.png"
                        )
                        try:
                            with open(clean_snapshot_path, "wb") as p:
                                p.write(png_bytes)
                        except Exception as e:
                            logger.error(f"Failed to save clean snapshot to {clean_snapshot_path}: {e}")
