...
class EmbeddingMaintainer(threading.Thread):
    ...
    def _process_finalized(self) -> None:
        ...
        while True:
            ...
            if updated_db:
                ...
                # Embed the reid description
                self._embed_reid_description(event)
    ...
    def _embed_reid_description(self, event:Event) -> None:
        """Embed the reid description for an event."""

        if not self.config.semantic_search.enabled:
            return
        
        # 空值检查（可选）
        if not event.data.get("description"):
            logger.info("Empty description after processing for %s", event.id)
            return
