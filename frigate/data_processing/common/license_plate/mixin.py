...
class LicensePlateProcessingMixin:
    ...
    def _process_license_plate(
        ...
        """
        # Check if using HyperLPR
        if hasattr(self.model_runner, 'use_hyperlpr') and self.model_runner.use_hyperlpr:
            return self._process_license_plate_hyperlpr(camera, id, image)
        
        # Using PaddleOCR for license plate detection and recognition
        logger.info(f"{camera}: Using PaddleOCR for license plate detection and recognition")
        ...
        logger.info(f"{camera}: PaddleOCR detected {len(boxes)} license plate box(es)")
        ...
        if all_license_plates:
            ...
            if sorted_data:
                logger.info(f"{camera}: PaddleOCR processing completed, returning {len(sorted_data)} license plate(s)")
                return map(list, zip(*sorted_data))

        logger.info(f"{camera}: PaddleOCR processing completed, no valid license plates found")
        return [], [], []
    ...
    def _process_license_plate_hyperlpr(
        self, camera: str, id: str, image: np.ndarray
    ) -> Tuple[List[str], List[List[float]], List[int]]:
        """
        Process license plates using HyperLPR.
        
        Args:
            camera (str): Camera identifier.
            id (str): Event identifier.
            image (np.ndarray): The input image in which to detect and recognize license plates.

        Returns:
            Tuple[List[str], List[List[float]], List[int]]: Detected license plate texts, 
            character-level confidence scores for each plate, and areas of the plates.
        """
        try:
            # Use HyperLPR for detection and recognition
            logger.info(f"{camera}: Using HyperLPR3 for license plate detection and recognition")
            plates = self.model_runner.hyperlpr_model.detect_and_recognize(image)
            
            if not plates:
                logger.debug(f"{camera}: No license plates detected by HyperLPR")
                return [], [], []
            
            logger.info(f"{camera}: HyperLPR3 detected {len(plates)} license plate(s)")
            
            plate_texts = []
            confidences = []
            areas = []
            
            for plate_text, confidence, bbox in plates:
                # Filter by confidence threshold
                if confidence < self.lpr_config.recognition_threshold:
                    logger.debug(f"{camera}: Plate '{plate_text}' filtered due to low confidence: {confidence:.3f}")
                    continue
                
                # Filter by minimum plate length
                if len(plate_text) < self.lpr_config.min_plate_length:
                    logger.debug(f"{camera}: Plate '{plate_text}' filtered due to length: {len(plate_text)}")
                    continue
                
                # Filter by format if specified
                if self.lpr_config.format:
                    if not re.match(self.lpr_config.format, plate_text):
                        logger.debug(f"{camera}: Plate '{plate_text}' filtered due to format mismatch")
                        continue
                ...
                # Create confidence list for each character (simplified)
                char_confidences = [confidence] * len(plate_text)
                confidences.append(char_confidences)
                areas.append(area)
                
                logger.debug(f"{camera}: HyperLPR detected plate: '{plate_text}' (confidence: {confidence:.3f}, area: {area})")
            
            logger.info(f"{camera}: HyperLPR3 processing completed, returning {len(plate_texts)} license plate(s)")
            return plate_texts, confidences, areas
            
        except Exception as e:
            logger.error(f"{camera}: Error in HyperLPR processing: {e}")
            return [], [], []
