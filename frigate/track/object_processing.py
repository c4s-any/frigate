...
from frigate.const import UPDATE_CAMERA_ACTIVITY, MODEL_P_REID_PATH, MODEL_V_REID_PATH
from frigate.events.types import EventStateEnum, EventTypeEnum
from frigate.models import Event, Timeline
from frigate.track.tracked_object import TrackedObject
from frigate.util.image import SharedMemoryFrameManager

from frigate.detectors.plugins.reid import InferenceEngine
import frigate.detectors.plugins.hyperlpr3.hyperlpr3 as lpr3
...
class TrackedObjectProcessor(threading.Thread):
    def __init__(
        ...
        def create_engine(config, default_path):
            return InferenceEngine(config.model_path or default_path, config.thread_num)

        engine_p = create_engine(self.preid_config, MODEL_P_REID_PATH)
        engine_v = create_engine(self.vreid_config, MODEL_V_REID_PATH)

        lpr_catcher = lpr3.LicensePlateCatcher()
        ...
        def end(camera: str, obj: TrackedObject, frame_name: str):
            ...
            if obj.has_snapshot:
                ...
                if label == "person" and self.preid_config.enabled:
                    obj.write_reid_snapshot_to_disk(engine_p, lpr_catcher)
                elif label == "car" and self.vreid_config.enabled:
                    obj.write_reid_snapshot_to_disk(engine_v, lpr_catcher)
                else:
                    obj.write_snapshot_to_disk()
    ...
    def set_description(
        self, event_id: str, description: str | None, score: float | None
    ) -> None:
        """Update recognized license plate for given event id."""
        tracked_obj: TrackedObject = None
        ...
        if tracked_obj:
            tracked_obj.obj_data["description"] = (
                description,
                score,
            )
        ...
        if event:
            ...
            if description is None:
                data["description_score"] = None
            elif score is not None:
                data["description_score"] = score
            event.data = data
            event.save()
    ...
    def run(self):
        while not self.stop_event.is_set():
            ...
            while True:
                ...
                if topic.endswith(EventMetadataTypeEnum.description.value):
                    (event_id, description, score) = payload
                    self.set_description(
                        event_id, description, score
                    )
