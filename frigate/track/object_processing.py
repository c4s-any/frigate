...
class TrackedObjectProcessor(threading.Thread):
    def __init__(
        ...
        # 获取ReID配置
        self.preid_config: pReIDConfig = self.config.preid
        self.vreid_config: vReIDConfig = self.config.vreid
        self.embeddings_metrics = embeddings_metrics
        # Initialize ReID session
        def create_engine(config, default_path, metrics=None):
            return InferenceEngine(config.model_path or default_path, config.thread_num, metrics)

        engine_p = create_engine(self.preid_config, MODEL_P_REID_PATH, embeddings_metrics)
        engine_v = create_engine(self.vreid_config, MODEL_V_REID_PATH, embeddings_metrics)
        ...
        def end(camera: str, obj: TrackedObject, frame_name: str):
            ...
            if obj.has_snapshot:
                ...
                if label == "person" and self.preid_config.enabled:
                    obj.write_reid_snapshot_to_disk(engine_p)
                elif label == "car" and self.vreid_config.enabled:
                    obj.write_reid_snapshot_to_disk(engine_v)
                else:
                    obj.write_snapshot_to_disk()
    ...
    def save_reid_snapshot(self, payload: tuple) -> None:
        """保存 ReID 快照"""
        (snapshot_data, event_id, camera) = payload
        ...
        try:
            ...
            img = cv2.imdecode(
                np.frombuffer(base64.b64decode(snapshot_data), dtype=np.uint8),
                cv2.IMREAD_COLOR,
            )
            ...
            logger.debug(f"Saved ReID snapshot for event {event_id} on camera {camera}")
            
        except Exception as e:
            logger.error(f"Failed to save ReID snapshot for event {event_id}: {e}")

    def create_reid_event(self, payload: tuple) -> None:
        """创建 ReID 事件"""
        (
            frame_time,
            camera_name,
        ...
        logger.debug(f"Created ReID event {event_id} for {label} on camera {camera_name}")

    def end_reid_event(self, payload: tuple) -> None:
        """结束 ReID 事件"""
        ...
        try:
            ...
            event.save()
            logger.debug(f"Ended ReID event {event_id}")
        except DoesNotExist:
            logger.warning(f"ReID event {event_id} not found for ending")
        except Exception as e:
            logger.error(f"Failed to end ReID event {event_id}: {e}")
    ...
    def run(self):
        while not self.stop_event.is_set():
            ...
            while True:
                ...
                elif topic.endswith(EventMetadataTypeEnum.preid_event_create.value):
                    self.create_reid_event(payload)
                elif topic.endswith(EventMetadataTypeEnum.vreid_event_create.value):
                    self.create_reid_event(payload)
                elif topic.endswith(EventMetadataTypeEnum.save_reid_snapshot.value):
                    self.save_reid_snapshot(payload)
                elif topic.endswith(EventMetadataTypeEnum.reid_event_end.value):
                    self.end_reid_event(payload)
