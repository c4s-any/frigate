...
class TrackedObject:
    ...
    def update(self, current_frame_time: float, obj_data, has_valid_frame: bool):
        ...
        if not self.false_positive and has_valid_frame:
            ...
            if self.thumbnail_data is None or (
                ...
                self.thumbnail_data = {
                    ...
                    "description": obj_data.get(
                        "description"
                    ),
                    "description_score": obj_data.get(
                        "description_score"
                    ),
                }
                thumb_update = True
        ...
        if self.obj_data["label"] in ["car", "motorcycle", "person"]:
            recognized_logos = {
    ...
    def to_dict(self):
        event = {
            ...
            "description": self.obj_data.get("description"),
        }
    ...
    def get_person_vehicle_characters(self, engine: InferenceEngine, lprcatcher):
    ...
    def get_img_bytes(
        ...
        if ext == "jpg":
            quality_params = [int(cv2.IMWRITE_JPEG_QUALITY), quality or 95]
        elif ext == "webp":
            quality_params = [int(cv2.IMWRITE_WEBP_QUALITY), quality or 95]
    ...
    def write_reid_snapshot_to_disk(self, engine, lprcatcher) -> None:
        snapshot_config: SnapshotsConfig = self.camera_config.snapshots

        characters_jpg_bytes = self.get_person_vehicle_characters(engine, lprcatcher)
