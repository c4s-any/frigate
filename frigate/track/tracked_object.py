...
from frigate.embeddings.reid import InferenceEngine
...
class TrackedObject:
    ...
    def get_person_vehicle_characters(self, engine: InferenceEngine):
        
        # 如果 thumbnail_data 为空，直接返回
        if self.thumbnail_data is None:
            return None
        ...
        # 调用 get_pv_bestFrame 处理帧
        jpg_bytes = engine.get_pv_bestFrame(
            self,
            frame,
            engine
        )
    ...
    def get_img_bytes(
        ...
        if ext == "jpg":
            quality_params = [int(cv2.IMWRITE_JPEG_QUALITY), quality or 96]
        elif ext == "webp":
            quality_params = [int(cv2.IMWRITE_WEBP_QUALITY), quality or 96]
    ...
    def write_reid_snapshot_to_disk(self, engine) -> None:
        ...
        # 检查是否是第一次调用（包含初始化时间）
        is_first_call = not hasattr(engine, '_initialization_complete')
        
        characters_jpg_bytes = self.get_person_vehicle_characters(engine)
        
        # 计算ReID推理时间
        reid_inference_time = time.time() - reid_start_time
        
        # 如果是第一次调用，不记录推理时间（因为包含初始化时间）
        if is_first_call:
            reid_inference_time = 0.0  # 第一次调用不记录时间

        if characters_jpg_bytes is None:
            logger.warning(f"Unable to save snapshot for {self.obj_data['id']}.")
        else:
            snapshot_path = os.path.join(CLIPS_DIR, f"{self.camera_config.name}-{self.obj_data['id']}.jpg")
            try:
                with open(snapshot_path, "wb") as j:
                    j.write(characters_jpg_bytes)
            except Exception as e:
                logger.error(f"Failed to save snapshot to {snapshot_path}: {e}")

        # 发布 ReID 事件（如果有特征识别结果）
        if hasattr(self.obj_data, 'get') and self.obj_data.get("sub_label"):
            try:
                from frigate.comms.event_metadata_updater import EventMetadataPublisher, EventMetadataTypeEnum
                import datetime
                import base64
                
                # 创建事件发布器
                event_publisher = EventMetadataPublisher()
                ...
                event_id = f"reid_{now}-{rand_id}"
                
                # 获取特征信息
                sub_label = self.obj_data.get("sub_label")
                description = self.obj_data.get("description", sub_label)
                label = self.obj_data.get("label", "unknown")
                
                # 计算分数
                if isinstance(sub_label, tuple):
                    max_score = sub_label[1]
                    sub_label_text = sub_label[0]
                else:
                    max_score = 0.8  # 默认分数
                    sub_label_text = str(sub_label)
                
                # 发布事件创建
                event_create_type = EventMetadataTypeEnum.preid_event_create if label == "person" else EventMetadataTypeEnum.vreid_event_create
                event_publisher.publish(
                    event_create_type,
                    (now, self.camera_config.name, label, event_id, True, max_score, sub_label_text, sub_label_text)
                )
                
                # 发布子标签事件
                event_publisher.publish(
                    EventMetadataTypeEnum.sub_label,
                    (event_id, sub_label_text, max_score)
                )
                
                # 发布快照保存事件
                if characters_jpg_bytes:
                    event_publisher.publish(
                        EventMetadataTypeEnum.save_reid_snapshot,
                        (base64.b64encode(characters_jpg_bytes).decode("ASCII"), event_id, self.camera_config.name)
                    )
                
                # 结束事件
                event_publisher.publish(
                    EventMetadataTypeEnum.reid_event_end,
                    (event_id, datetime.datetime.now().timestamp())
                )
                
                # 更新ReID性能指标（仅当有特征识别结果时）
                if hasattr(engine, 'metrics') and engine.metrics:
                    if label == "person":
                        engine.metrics.preid_speed.value = reid_inference_time
                        # 更新每秒处理数（这里可以添加更复杂的逻辑）
                        engine.metrics.preid_pps.value = 1.0 / reid_inference_time if reid_inference_time > 0 else 0.0
                    elif label == "car":
                        engine.metrics.vreid_speed.value = reid_inference_time
                        # 更新每秒处理数（这里可以添加更复杂的逻辑）
                        engine.metrics.vreid_pps.value = 1.0 / reid_inference_time if reid_inference_time > 0 else 0.0
                
                logger.debug(f"Published ReID event {event_id} for {label} with features: {sub_label_text}, inference time: {reid_inference_time:.3f}s")
                
            except Exception as e:
                logger.warning(f"Failed to publish ReID event: {e}")
