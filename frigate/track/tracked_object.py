...
from frigate.track.reid import get_pv_bestFrame
...
class TrackedObject:
    ...
    def update(self, current_frame_time: float, obj_data, has_valid_frame: bool):
        ...
        if self.obj_data["label"] in ["机动车", "package", "人"]:
            recognized_logos = {
    ...
    def get_person_vehicle_attr(self, reid_session_p, reid_session_v, lp_enabled, p_enabled, v_enabled, shorter_size, longer_size, min_score_lp, min_score_p, min_score_v):
    ...
    def get_jpg_bytes(
        self, timestamp=False, bounding_box=False, crop=False, height=None, quality=95
    ):
        if self.thumbnail_data is None:
            return None
    ...
    def find_best_object(self, objects: list[dict[str, any]]) -> Optional[str]:
        ...
        for obj in objects:
            ...
            if best_object_area is None:
                ...
                if best_object_label == "机动车" and obj["label"] == "机动车":
