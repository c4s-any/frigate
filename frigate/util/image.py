...
def draw_chinese_text_on_image(img: np.ndarray, text: str, position: tuple, 
    ...
    Args:
        ...
        text: Text to draw (can contain Chinese characters)
    ...
    if not PIL_AVAILABLE:
        # Fallback to transliterated text with OpenCV
        display_text = transliterate_to_latin(text)
        cv2.putText(img, display_text, position, cv2.FONT_HERSHEY_SIMPLEX, 
    ...
    try:
        ...
        logger.warning(f"Failed to draw Chinese text with PIL: {e}, falling back to transliterated text")
        # Fallback to transliterated text
        display_text = transliterate_to_latin(text)
        cv2.putText(img, display_text, position, cv2.FONT_HERSHEY_SIMPLEX, 
                   font_size/40.0, color, 2)
        return img
...
def draw_box_with_label(
    ...
    if color is None:
        color = (0, 0, 255)
    # 检查是否包含中文字符
    full_text = "{}: {}".format(label, info)
    has_chinese = any('\u4e00' <= char <= '\u9fff' for char in full_text)
    
    if has_chinese and PIL_AVAILABLE:
        # 使用中文显示
        display_text = full_text
    else:
        # 使用拉丁字符转换
        try:
            display_text = transliterate_to_latin(full_text)
        except Exception:
            display_text = full_text
    ...
    if has_chinese and PIL_AVAILABLE:
        # 使用PIL绘制中文
        text_position = (text_offset_x, text_offset_y + line_height - 3)
        frame = draw_chinese_text_on_image(
            frame, 
            display_text, 
            text_position, 
