...
def is_better_thumbnail(label, current_thumb, new_obj, frame_shape) -> bool:
    ...
    # check face or glasses on person
    if label == "人":
        if has_better_attr(current_thumb, new_obj, "face"):
            return True
        if has_better_attr(current_thumb, new_obj, "眼镜"):
            return True
        # if the current thumb has a face or glasses attr, dont update unless it gets better
        if any([a["label"] == "face" or a["label"] == "眼镜" for a in current_thumb["attributes"]]):
            return False

    # check license_plate on car
    if label == "机动车":
        if has_better_attr(current_thumb, new_obj, "license plate"):
            return True
        # if the current thumb has a license_plate attr, dont update unless it gets better
        if any([a["label"] == "license plate" for a in current_thumb["attributes"]]):
            return False
