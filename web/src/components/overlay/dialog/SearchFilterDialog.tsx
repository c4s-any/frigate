...
export function SubFilterContent({
  ...
  const { t } = useTranslation(["components/filter"]);
  const [selectedSubLabels, setSelectedSubLabels] = useState<string[]>(subLabels || []);
  const [inputValue, setInputValue] = useState("");

  useEffect(() => {
    if (subLabels) {
      setSelectedSubLabels(subLabels);
    } else {
      setSelectedSubLabels([]);
    }
  }, [subLabels]);

  const handleSelect = (subLabel: string) => {
    const newSelected = selectedSubLabels.includes(subLabel)
      ? selectedSubLabels.filter((id) => id !== subLabel) // Deselect
      : [...selectedSubLabels, subLabel]; // Select

    setSelectedSubLabels(newSelected);
    if (newSelected.length === 0) {
      setSubLabels(undefined); // Clear filter if no labels selected
    } else {
      setSubLabels(newSelected);
    }
  };

  if (allSubLabels && allSubLabels.length === 0) {
    return null;
  }
  ...
  return (
    <div className="overflow-x-hidden">
      <DropdownMenuSeparator className="mb-3" />
      <div className="mb-3 text-lg">{t("subLabels.label")}</div>
      {allSubLabels == undefined ? (
        <div className="flex flex-col items-center justify-center text-sm text-muted-foreground">
          <ActivityIndicator className="mb-3 mr-2 size-5" />
          <p>{t("subLabels.loading", "Loading features...")}</p>
        </div>
      ) : allSubLabels.length == 0 ? null : (
        <>
          <Command
          ...
          >
            <CommandInput
              placeholder={t("subLabels.searchPlaceholder", "Search features...")}
              value={inputValue}
              onValueChange={setInputValue}
            />
            <CommandList className="max-h-[200px] overflow-auto">
              {allSubLabels.length > 0 && inputValue && (
                <CommandEmpty>
                  {t("subLabels.noResults", "No features found")}
                </CommandEmpty>
              )}
              {allSubLabels.map((label) => (
                ...
                >
                  <LuCheck
                    className={cn(
                      "mr-2 h-4 w-4",
                      selectedSubLabels.includes(label)
                        ? "opacity-100"
                        : "opacity-0",
                    )}
                  />
