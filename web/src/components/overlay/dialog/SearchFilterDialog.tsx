...
export function SubFilterContent({
  ...
  const handleSelect = (label: string) => {
    const newSelected = selectedSubLabels.includes(label)
      ? selectedSubLabels.filter((id) => id !== label)
  ...
  const filteredSubLabels = allSubLabels.filter((label) =>
  ...
  return (
    <div className="overflow-x-hidden">
      ...
      <Command className="border border-input bg-background">
        <CommandInput
          placeholder={t("subLabels.placeholder")}
          value={inputValue}
          onValueChange={setInputValue}
        />
        <CommandList className="max-h-[200px] overflow-auto">
          {filteredSubLabels.length === 0 && inputValue && (
            <CommandEmpty>
              {t("subLabels.noLabelsFound")}
            </CommandEmpty>
          )}
          {filteredSubLabels.map((label) => (
            <CommandItem
              key={label}
              value={label}
