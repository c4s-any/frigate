...
export default function ClassificationSettingsView({
  ...
  const [classificationSettings, setClassificationSettings] =
    useState<ClassificationSettings>({
      ...
      hyperlpr: { enabled: undefined },
      preid: { enabled: undefined },
      vreid: { enabled: undefined },
    });
    ...
    if (config) {
      if (classificationSettings?.search.enabled == undefined) {
        setClassificationSettings({
          ...
          hyperlpr: { enabled: config.hyperlpr.enabled },
          preid: { enabled: config.preid.enabled },
          vreid: { enabled: config.vreid.enabled },
        });
      }
  ...
  const handleClassificationConfigChange = (
    ...
    setClassificationSettings((prevConfig) => ({
      ...
      hyperlpr: { ...prevConfig.hyperlpr, ...newConfig.hyperlpr },
      preid: { ...prevConfig.preid, ...newConfig.preid },
      vreid: { ...prevConfig.vreid, ...newConfig.vreid },
    }));
  ...
  const saveToConfig = useCallback(async () => {
    ...
    axios
      .put(
        `config/set?semantic_search.enabled=${classificationSettings.search.enabled ? "True" : "False"}&semantic_search.model_size=${classificationSettings.search.model_size}&face_recognition.enabled=${classificationSettings.face.enabled ? "True" : "False"}&face_recognition.model_size=${classificationSettings.face.model_size}&hyperlpr.enabled=${classificationSettings.hyperlpr.enabled ? "True" : "False"}&preid.enabled=${classificationSettings.preid.enabled ? "True" : "False"}&vreid.enabled=${classificationSettings.vreid.enabled ? "True" : "False"}`,
        { requires_restart: 0 },
      )
      .then((res) => {
        if (res.status === 200) {
        ...
        <div className="my-2 space-y-6">
          ...
          <div className="max-w-6xl">
            <div className="mb-5 mt-2 flex max-w-5xl flex-col gap-2 text-sm text-primary-variant">
              ...
              <div className="flex items-center text-primary">
                <Link
                  to="https://c4s.tech/docs/Frigate/configuration/face_recognition"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline"
                >
          ...
          <Heading as="h4" className="my-2">
            {t("classification.hyperLpr.title")}
          </Heading>
          <div className="max-w-6xl">
            <div className="mb-5 mt-2 flex max-w-5xl flex-col gap-2 text-sm text-primary-variant">
              <p>{t("classification.hyperLpr.desc")}</p>

              <div className="flex items-center text-primary">
                <Link
                  to="https://c4s.tech/docs/Frigate/configuration/license_plate_recognition"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline"
                >
                  {t(
                    "classification.hyperLpr.readTheDocumentation",
                  )}
                  <LuExternalLink className="ml-2 inline-flex size-3" />
                </Link>
              </div>
          ...
          <div className="flex w-full max-w-lg flex-col space-y-6">
            <div className="flex flex-row items-center">
              <Switch
                id="enabled"
                className="mr-3"
                disabled={classificationSettings.hyperlpr.enabled === undefined}
                checked={classificationSettings.hyperlpr.enabled === true}
                onCheckedChange={(isChecked) => {
                  handleClassificationConfigChange({
                    hyperlpr: { enabled: isChecked },
                  });
                }}
              />
              <div className="space-y-0.5">
          ...
          <Heading as="h4" className="my-2">
            {t("classification.preid.title")}
          </Heading>
          <div className="max-w-6xl">
            <div className="mb-5 mt-2 flex max-w-5xl flex-col gap-2 text-sm text-primary-variant">
              <p>{t("classification.preid.desc")}</p>

              <div className="flex items-center text-primary">
                <Link
                  to="https://c4s.tech/docs/Frigate/configuration/reid"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline"
                >
                  {t(
                    "classification.preid.readTheDocumentation",
                  )}
                  <LuExternalLink className="ml-2 inline-flex size-3" />
                </Link>
              </div>
          ...
          <div className="flex w-full max-w-lg flex-col space-y-6">
            <div className="flex flex-row items-center">
              <Switch
                id="enabled"
                className="mr-3"
                disabled={classificationSettings.preid.enabled === undefined}
                checked={classificationSettings.preid.enabled === true}
                onCheckedChange={(isChecked) => {
                  handleClassificationConfigChange({
                    preid: { enabled: isChecked },
                  });
                }}
              />
              <div className="space-y-0.5">
          ...
          <Heading as="h4" className="my-2">
            {t("classification.vreid.title")}
          </Heading>
          <div className="max-w-6xl">
            <div className="mb-5 mt-2 flex max-w-5xl flex-col gap-2 text-sm text-primary-variant">
              <p>{t("classification.vreid.desc")}</p>
              ...
              <div className="flex items-center text-primary">
                ...
                >
                  {t(
                    "classification.vreid.readTheDocumentation",
                  )}
                  <LuExternalLink className="ml-2 inline-flex size-3" />
                </Link>
              </div>
          ...
          <div className="flex w-full max-w-lg flex-col space-y-6">
            <div className="flex flex-row items-center">
              <Switch
                id="enabled"
                className="mr-3"
                disabled={classificationSettings.vreid.enabled === undefined}
                checked={classificationSettings.vreid.enabled === true}
                onCheckedChange={(isChecked) => {
                  handleClassificationConfigChange({
                    vreid: { enabled: isChecked },
                  });
                }}
              />
              <div className="space-y-0.5">
