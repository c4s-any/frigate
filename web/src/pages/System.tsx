...
function System() {
  ...
  useEffect(() => {
    if (pageToggle) {
      document.title = `${en2zh(pageToggle)}统计 - Frigate`;
    }
  }, [pageToggle]);
        ...
        >
          {Object.values(metrics).map((item) => (
            ...
            >
              ...
              {isDesktop && <div className="capitalize">{en2zh_item(item)}</div>}
            </ToggleGroupItem>
          ))}
        </ToggleGroup>
        ...
        <div className="flex h-full items-center">
          {lastUpdated && (
            <div className="h-full content-center text-sm text-muted-foreground">
              最后刷新： <TimeAgo time={lastUpdated * 1000} dense />
            </div>
          )}
        </div>
      </div>
      <div className="mt-2 flex items-end gap-2">
        <div className="h-full content-center font-medium">系统</div>
        {statsSnapshot && (
          <div className="h-full content-center text-sm text-muted-foreground">
