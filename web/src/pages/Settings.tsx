...
export default function Settings() {
  const [page, setPage] = useState<SettingsType>("用户界面设置");
    ...
    <div className="flex size-full flex-col p-2">
          ...
          <div ref={tabsRef} className="flex flex-row">
            ...
            >
              {Object.values(settingsViews).map((item) => (
                <ToggleGroupItem
                  key={item}
                  className={`flex scroll-mx-10 items-center justify-between gap-2 ${page == "用户界面设置" ? "last:mr-20" : ""} ${pageToggle == item ? "" : "*:text-muted-foreground"}`}
                  value={item}
                  data-nav-item={item}
      ...
      {confirmationDialogOpen && (
        ...
        >
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>您有未保存的更改。</AlertDialogTitle>
              <AlertDialogDescription>
                继续之前要保存更改吗？
              </AlertDialogDescription>
            </AlertDialogHeader>
