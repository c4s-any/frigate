    ...
    >
      <div className="flex flex-row justify-between">
        <div className="flex flex-col gap-1">
          <div className="select-none smart-capitalize min-w-0">
            {t("details.person")}
            {event?.sub_label ? (
              <Tooltip>
                <TooltipTrigger asChild>
                  <span className="truncate max-w-[200px]" title={event.sub_label}>
                    : {event.sub_label} ({Math.round((event.data.sub_label_score || 0) * 100)}%)
                  </span>
                </TooltipTrigger>
                <TooltipPortal>
                  <TooltipContent>
                    <p className="max-w-[300px] break-words">{event.sub_label}</p>
                  </TooltipContent>
                </TooltipPortal>
              </Tooltip>
            ) : (
